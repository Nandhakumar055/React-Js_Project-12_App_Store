...
class AppStore extends Component {
  ...
  clickTebItem = clickId => {
    this.setState({activeTabId: clickId})
  }

  getFilterAppItems = searchApp => {
    const {activeTabId} = this.state
    const filterAppItem = searchApp.filter(
      eachItem => eachItem.category === activeTabId,
    )
    return filterAppItem
  }

  getSearchResult = () => {
    const {searchInput} = this.state
    const searchResult = appsList.filter(eachResultItem =>
      eachResultItem.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return searchResult
  }
  ...
  render() {
    ...
    const filterAppItem = this.getFilterAppItems(searchResult)
      ...
      <div className="app-store-bg-container">
        ...
        <ul className="tab-item-container">
          {tabsList.map(eachTabItem => (
            <TabItem
              tabItems={eachTabItem}
              key={eachTabItem.tabId}
              clickTebItem={this.clickTebItem}
